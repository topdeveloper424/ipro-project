...
module.exports.getUserFeeds = function (chatpage, socket, io, pool,async)
{
	socket.on('senddata', function (data)
    {
        ...
        console.log(online_users.includes(data.user_id));
        if( online_users.includes(data.user_id) == false ){        	
        	online_users.push(data.user_id);
        }
        console.log(online_users);
        console.log(online_users.join(",") );
		pool.getConnection(function (err, connection)
		{
			 async.parallel([
				function(callback)
				{
					connection.query('SELECT id as user_id, avatar, username FROM users where id=' + data.user_id + '', function (error1, userdata)
					{
				...
				{					
					if(data.user_id)						
					connection.query('SELECT id as user_id, avatar, username FROM users where id in (' + online_users.join(",") + ')', function (error3, memdata)
					{
				...
				{
					if(data.user_id)
					connection.query('SELECT comment_id,comment,users.id as user_id, avatar, username,comments.added FROM comments INNER JOIN users ON comments.user_id=users.id',function(error4,converdata){
		...
		pool.getConnection(function (err, connection)
        {
			connection.query('INSERT INTO comments (user_id,comment,added) VALUES (' + data.user_id + ',"' + data.msg + '","' + data.datetime + '")', function (err, result)
            {
				...
				async.parallel([
					function (callback)
					{
						connection.query('SELECT comments.*, avatar, username,comments.added from comments JOIN users ON comments.user_id=users.id WHERE comments.comment_id=' + result.insertId + '', function (err2, comments)
						{
